...
export default function QuotationsPage() {
  ...
  const [form, setForm] = useState({ rfqId: '', price: '', deliveryDays: '', notes: '' });
  ...
  useEffect(() => {
    if (token) {
      ...
      if (user?.role === 'vendor') {
        api('/rfq', { token })
          .then((data: RFQ[]) => setRfqs(data.filter(r => r.status === 'OPEN')))
          .catch(console.error);
      }
    }
  }, [token, user?.role]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newQuotation = await api('/quotations', {
        method: 'POST',
        body: { ...form, price: parseFloat(form.price), deliveryDays: parseInt(form.deliveryDays) },
        token,
      });
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '600', color: 'var(--slate-800)' }}>Quotations</h1>
          <p style={{ fontSize: '14px', color: 'var(--slate-500)', marginTop: '4px' }}>
            {user?.role === 'vendor' ? 'Submit quotes for assigned RFQs' : 'Review and compare vendor quotations'}
          </p>
        </div>
      ...
      <div className="card">
        {quotations.length === 0 ? (
          <p style={{ color: 'var(--slate-500)', fontSize: '14px' }}>No quotations yet</p>
        ) : (
          <table className="table">
            ...
            <tbody>
              {quotations.map((q) => (
                <tr key={q._id}>
                  <td>
                    <Link href={`/rfq/${q.rfqId?._id}`} style={{ color: 'var(--accent)', fontWeight: '500' }}>
                      {q.rfqId?.title || 'Unknown'}
                    </Link>
                  </td>
                  {user?.role !== 'vendor' && <td>{q.vendorId?.name || 'Unknown'}</td>}
                  <td style={{ fontWeight: '500' }}>Rs. {q.price.toLocaleString()}</td>
                  <td>{q.deliveryDays} days</td>
                  <td>
                    <span className={`badge ${getStatusBadge(q.status)}`}>{q.status}</span>
                  </td>
